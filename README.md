...
| **1. Music** | Procedural step-sequencer — pure theory (`noteToFrequency`, `buildScale`, `swing`), seeded `generatePattern`, pure `advanceSequencer` seam, host `createSequencer` reusing `AudioAdapter` (two-clock lookahead scheduler) | **Shipped** |
...
| **2. Level schema** | Versioned platformer level format with discriminated-union entity taxonomy (spawn/exit/platform/trap/hazard/trigger/movingPlatform/collectible), closed `CollectibleKind` (`coin|gem|key`) sub-dispatch, forward-ladder migration, defensive validation, `TileSolidityQuery` bridge, canonical JSON + FNV-1a hashing. `validateLevel`, `migrateLevel`, `createTileQuery` | **Shipped** |
