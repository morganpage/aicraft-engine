# Spin Loop — A Seven-Beat Momentum-Sonic Act on `aicraft-engine@0.16.0`
...
**Non-negotiable: build the entire game on top of `aicraft-engine@0.16.0`.** Do not hand-roll the controller, fixed-step loops, AABB collision, cameras, ring pickups, particle bursts, jump arcs, palettes, or audio. If you find yourself writing a circular-collision routine for the loop, a second vertical-velocity integrator, a `Math.random()` in the simulation, or a `CLASSIC_PLATFORMER` import, STOP and use the engine instead. Spin Loop's headline is **momentum + signed gravity**, not dash-precision — Celerock owns that lane.
