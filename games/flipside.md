# Flipside — A Six-Room Gravity-Flip Showcase on `aicraft-engine@0.17.2`
...
**Non-negotiable: build the entire game on top of `aicraft-engine@0.17.2`.** Do not hand-roll fixed-step loops, AABB collision, cameras, tile rendering, joypad input, particle bursts, the music sequencer, audio synthesis, or chiptune OSC graphs. If you find yourself writing a `requestAnimationFrame` accumulator, a gravity-flip velocity integrator, an `OscillatorNode` graph, a hand-drawn tile renderer, or a `Math.random()` in the simulation, stop and use the engine instead.
...
- **`aicraft-engine@0.17.2`** is your only runtime dependency. Import **only** from the root barrel:
  ```ts
  import {
    // primitives
