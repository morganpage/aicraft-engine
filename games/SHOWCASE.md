...
| **[Celerock](./celerock.md)** | Single-screen precision platformer | *Celeste* | ★★★ | ~25 min | ~1200 | [`platformer`](../src/platformer/index.ts) kernel (`defaultPrecisionPipeline`: jump + wall-slide + dash) |
...
_Difficulty key: ★ = your first prompt; ★★★★ = you've built three others first. Flipside is rated ★★ (not ★★★) because its kernel is `PUZZLE_PLATFORMER` with an empty ability pipeline — the hard part (the music determinism seam) is bounded by `advanceSequencer`'s pure contract, not the kernel._
...
2. **[Celerock](./celerock.md)** — tightens kernel understanding: you swap in `PRECISION_PLATFORMER` and the `defaultPrecisionPipeline` jumps to the **ability pipeline**, adding `dashAbility` + `wallSlideAbility` + `doubleJumpAbility` to the familiar jump.
   You also pick up the `game-state` FSM (`createGameState` + `reduceGameState` + `isLegalTransition`) for `menu → playing → levelComplete`.
...
| **0.4.0 `tileTypeMap`** — unified tile + entity `compileLevel` | [World 1-1](./world-1-1.md) | [Flipside](./flipside.md) | 5 of 7 compile via `tileTypeMap`; Embertomb uses legacy `createTileQuery`; Doodle Knight has no tiles. |
...
| [`camera`](../src/camera/index.ts) (`createCamera` / `updateCamera`) | [World 1-1](./world-1-1.md) | [Doodle Knight](./doodle-knight.md) | All 7 use it. Doodle Knight's vertical-clamp (follows UP only) is the unique variant; Spin Loop adds speed-scaled lookahead. |
...
| [`easing`](../src/easing/index.ts) — Penner curves + `createTweenState` / `advanceTween` | [World 1-1](./world-1-1.md) | [Celerock](./celerock.md) | World 1-1: floating score popups (`easeOutCubic` + `easeOutBack`). Celerock: room-title fade + "Cleared" card. |
...
| [`level`](../src/level/index.ts) schema — `LevelData` / `validateLevel` / `migrateLevel` | [World 1-1](./world-1-1.md) | [Flipside](./flipside.md) | 5 of 7 author `LevelData`; Embertomb uses a raw tile grid; Doodle Knight has no tiles. |
...
- If you've built **[Celerock](./celerock.md)**, **[Bosscard](./bosscard.md)** will teach you little new about the kernel.
  Both run `PRECISION_PLATFORMER` + `defaultPrecisionPipeline`. The new pieces Bosscard adds (custom `createEnemyBehaviorRegistry`, `sampleConeVelocity` bullets, parry mechanic) are narrow.
...
- **[Celerock](./celerock.md)** and **[Spin Loop](./spin-loop.md)** share `PRECISION_PLATFORMER`; Spin Loop's novelty is *signed gravity* (the two-controller loop-de-loop pattern), Celerock's is the *ability pipeline* (wall-slide + dash + wall-jump).
