# World 1-1 — A Ten-Beat Horizontal Platformer Slice on `aicraft-engine@0.17.0`
...
**Non-negotiable: build the entire game on top of `aicraft-engine@0.17.0`.** Do not hand-roll fixed-step loops, AABB collision, cameras, jump arcs, level compilation, tile rendering, parallax, particle bursts, or audio synthesis. If you find yourself writing a `requestAnimationFrame` accumulator, a tile renderer, a `Math.random()` in the simulation, a gravity integrator, or a pixel-art Mario, stop and use the engine instead.
