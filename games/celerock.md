...
1. Vite + TypeScript + `aicraft-engine@0.21.0`. Wire the loop via `recipes/fixed-tick-game.ts`'s `startFixedTickGame` — `createGameLoop` with the §2 `onError` handler and the reduced-motion gate in one call — so a throw can't silently freeze the loop.
