...
```bash
BASE=https://raw.githubusercontent.com/morganpage/aicraft-engine/v0.22.10/games
...
**The first measurement can race layout, and a resize without a re-snap is half a fix.** `getBoundingClientRect()` called SYNCHRONOUSLY at boot — before the very first layout pass has necessarily settled — can return a near-zero rect in a slow-loading or embedded host (a real automated run hit this reliably: a 1×1 viewport, `celesteCameraZoom` fitted to it, and the game rendered as an all-black frame with no error anywhere, because nothing in the render path *throws* on a degenerate viewport, it just draws nothing visible). A bare `window.addEventListener('resize', () => upkeepCanvas(game))` does not close this: `resize` never fires for the canvas's own box settling into its first real layout, only for the window changing size afterward — so the bug is invisible until a player resizes the window, if ever. And when a resize *does* fire, calling `upkeepCanvas` alone still isn't enough: `game.viewport` self-corrects, but `game.brain.zoom` — set once, by `snapCameraBrain` at boot, from whatever viewport was known then — does not, because nothing re-derives it outside `updateCameraBrain`'s own per-tick call. A state that skips the sim (the start menu, a game-over freeze, anything §8 adds that doesn't run the camera every tick) is left with a permanently wrong zoom even after the viewport itself is fine. **Wire a `ResizeObserver` on the canvas, not just a `window.resize` listener, and make both call ONE function that does the resize AND the re-snap together:**

```ts
function onCanvasResized(game: Game): void {
  upkeepCanvas(game);   // resizeCanvasToBackingStore + canvasCssViewport
  game.brain = snapCameraBrain(game.brain, cameraOptionsFor(game, DEFAULT_FIXED_DT));
}
// at boot, and on every later change:
onCanvasResized(game);
new ResizeObserver(() => onCanvasResized(game)).observe(canvas);
window.addEventListener('resize', () => onCanvasResized(game));  // DPR can change (moving monitors) without the canvas's own box changing
```

`ResizeObserver`'s own first callback fires once real layout is in, which is what makes the viewport self-correct instead of staying wrong for the whole session; the `window` listener stays alongside it for the one case `ResizeObserver` cannot see (a DPR change with no box-size change). Skipping the re-snap is the sneakier half of this bug — it passes every check that only looks at `game.viewport`.
...
- `menu → playing` via `{ type: 'start' }` on menu confirm. **The menu is a start menu with two entries — NEW GAME and RESUME GAME:**
  ...
  - **Space entries by the FONT's actual line height, not a guessed constant.** `drawTextOutlined` has no opinion on line spacing — it draws one string at the `(x, y)` you give it and nothing stops two calls from overlapping. `DEFAULT_TEXT_SCALE` is `3` and the default font is 7px tall, so one line of unscaled text is `7 × 3 = 21`px; a gap smaller than that (a real build used `12`) prints RETRY directly on top of QUIT — an unreadable smear, invisible in code review because every call site is correct in isolation, and caught only by actually looking at a captured pause-menu screenshot (§13 gate 13), never by the type checker. Compute the gap from the font (`(font.cellHeight + font.lineGap) × scale`, or just a value comfortably above `21` at the default scale) instead of a bare number.
...
The two checklist items above are the contract. **Both ship as copy-in recipes —
`recipes/wind-atmosphere.ts` and `recipes/backdrop-sky.ts` — so the normal path
is to copy them in and skip to §9.1.4's four rules.** The rest of this section
is why each constant is what it is, which you need before you retune anything.
It is lifted from the build that tuned it by ear against a live lab (`wind-lab.html` — a page that renders the sky and writes to the tuning
...
- `scanForbiddenIdentifiers('src')` returns `[]` — no `Math.random`, `Date.now`, `requestAnimationFrame`, or `fillText` in game code (the bitmap font is the only text path; a real build's boot-error `fillText` survived because the scan existed only as a manual grep nobody ran). Copy-in engine recipes under `src/recipes/` are excluded — they are engine-tested. **The scan is TEXT, not AST — it cannot tell a call from a comment.** A doc comment that explains a cache-busting query string by naming what NOT to do (`// never Date.now()`) fails the scan exactly as `fetch(url + Date.now())` would, because the substring is on the line either way. This is correct, not a false positive — the scan's whole point is catching the pattern by name — but it means the fix for a flagged comment is to rephrase it without the literal identifier, same as the fix for flagged code is to stop calling it.
...
section ships as a runnable script — [`games/celerock-wiring-check.sh`](https://github.com/morganpage/aicraft-engine/blob/v0.22.10/games/celerock-wiring-check.sh)
...
Both loops print nothing when clean. `particle-system` is never part of this
check — it is the pre-0.21.0 back-port, and at this brief's pin you call
`stepSeconds` directly, so a build that imports it is behind, not ahead.

`ldtk-entity-tile-art` is capability-gated **on the level, not on the stage**,
and the gate asks the level rather than assuming:

```bash
if [ "$STAGE" -ge 4 ] && grep -lq FallingBlock public/*.ldtk; then
  # ldtk-entity-tile-art must be imported, and advanceFallingBlocks wired.
fi
```

This distinction is load-bearing and was learned the expensive way. The shipped
pack gained a `FallingBlock` entity *after* the reference build shipped, so the
reference's `.ldtk` defines none and it imports no tile-art recipe — a flat
requirement would fail the build that passed §12.7. A build using the current
pack, however, must wire `advanceFallingBlocks` rather than hand-roll the
arm/shake/grace/fall/land/crush machine the engine owns (§12.8).
