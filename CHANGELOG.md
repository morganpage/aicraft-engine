...
- **`PlatformerConfig.dashCooldown` corrected 0.3 → 0.2 (physics v16) — matches Celeste exactly.** Celeste's `DashCooldown = .2f` (`Player.cs:79`); the old 0.3 had no source citation, unlike every other dash timing constant in `constants.ts`, so it read as drift rather than a deliberate choice. A dash-refill crystal only restores `dashesRemaining` — Celeste's own `RefillDash()` (`Player.cs:2002-2010`) never touches the cooldown timer either — so the longer cooldown widened the window where a fully-charged second dash press right after chaining through a crystal is silently swallowed. Trajectory-changing (a same-direction re-dash unlocks 0.1s sooner): every `replayHashFor` shifts, and `traceHash` shifts for any pinned scenario that presses dash again inside the old 0.2–0.3s window. 3 new tests. Exercised in `platformer-dash.test.ts`.
